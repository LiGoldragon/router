...
- an emitted async task-backed router daemon process shell generated by
  `schema-rust` under `src/schema/daemon.rs`;
...
- checked-in generated triad modules under `src/schema/`, produced from
  ...
  `schema-rust`. These modules expose router's Signal/Nexus/SEMA nouns
  and the active generated daemon shell. The live binary uses that generated
  multi-listener shell for argument parsing, listener binding, lifecycle, and
  working-vs-meta dispatch. The remaining hand-written daemon code is the
  component hook that decodes the working socket's two legacy relation
  contracts (`signal-message` ingress and `signal-router` observation frames)
  before routing into `RouterRuntime`;
